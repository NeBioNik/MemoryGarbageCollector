...
-- GetFormattedString
local GFS = function (STR_NAME, ...) return SF(GetString(STR_NAME), ...) end

local MGC = {}
MGC.author    = "|cbf16b9NeBioNik|r [@BioNik12]"
MGC.name      = "MemoryGarbageCollector"
MGC.fullName  = "Memory Garbage Collector"
MGC.shortName = "MGC"
MGC.prefix    = "|cbf16b9[MGC]|r"
...
function MGC:Init()
    self.config = ZO_SavedVars:NewAccountWide(self.name .. 'Vars', 1, nil, DEFAULT_SAVED_VARS)
...
function MGC:InitMenu()
    ...
    local optionsTable = {
        ...
        },
        {
            ...
            getFunc  = function() return self.config.autoClear end,
            setFunc  = function(v)
        ...
        },
        {
            ...
            max      = 60,
            decimals = 0,
            disabled = function() return not self.config.autoClear end,
            getFunc  = function() return self.config.refreshRate end,
            setFunc  = function(v)
        ...
        },
        {
            ...
            disabled = function() return not self.config.autoClear end,
            getFunc  = function() return self.config.comparisonMethod end,
            setFunc  = function (v) self.config.comparisonMethod = v end,
            choicesValues = {1, 2},
        ...
        },
        {
            ...
            step     = 5,
            decimals = 0,
            disabled = function() return not (self.config.autoClear and self.config.comparisonMethod == 1) end,
            getFunc  = function() return self.config.overflowRelative end,
            setFunc  = function(v)
        ...
        },
        {
            ...
            step     = 25,
            decimals = 0,
            disabled = function() return not (self.config.autoClear and self.config.comparisonMethod == 2) end,
            getFunc  = function() return self.config.overflowAbsolute end,
            setFunc  = function(v)
        ...
        },
        {
            ...
            disabled = function() return not self.config.autoClear end,
            getFunc  = function() return self.config.showDebugMessages end,
            setFunc  = function(v) self.config.showDebugMessages = v end,
...
function MGC:calcMaxMemory(memory)
    if not memory then memory = self:currentMemory() end
...
function MGC:checkGarbage()
    ...
    if current <= limit then return end
...
function MGC.OnAddOnLoaded(event, addonName)
    if addonName ~= MGC.name then return end
